...
var bracketRegex = /\[\w+\]/g;
var abbaRegex = /(\w)(\w)\2\1/g;
var quadRegex = /(\w)\1{3}/;

function hasABBA(str) {
    var matches = str.match(abbaRegex);
    return matches && matches.filter(match => !quadRegex.test(match)).length;
}

function countTLSsupport(text) {
 return text
    .split('\n')
    .filter(line => {
        var bracketMatches = line.match(bracketRegex);
        var blocksTLS = false;
    ...
    .filter(line => hasABBA(line))
    .length;
}

console.log(countTLSsupport('abba[mnop]qrst') === 1);
console.log(countTLSsupport('abcd[bddb]xyyx') === 0);
console.log(countTLSsupport('aaaa[qwer]tyui') === 0);
console.log(countTLSsupport('ioxxoj[asdfgh]zxcvbn') === 1);
console.log(countTLSsupport(input));
