var input = `bot 147 gives low to bot 67 and high to bot 71
...
var botRegex = /bot (\d+) gives low to (output|bot) (\d+) and high to (output|bot) (\d+)/;
var pushValRegex = /value (\d+) goes to bot (\d+)/;

var queuedActions = [];

class Bot {
    constructor(botNumber, pushLow, pushHigh) {
        this.botNumber = botNumber;
        this.pushLow = pushLow;
        this.pushHigh = pushHigh;
        this.values = [];
    }

    pushValue(value) {
        console.log(`bot ${this.botNumber} receiving value ${value}`);
        this.values.push(value);

        if(this.values.length == 2) {
            this.values.sort((a,b) => a - b);

            var low = this.values[0];
            var high = this.values[1];

            // Advent of Code answer
            if (low === 17 && high === 61) {
                throw new Error(`Answer is ${this.botNumber}`);
            }

            queuedActions.push(() => {
                console.log(`bot ${this.botNumber} pushing low value ${low}`);
                this.pushLow(low);
            });
            queuedActions.push(() => {
                console.log(`bot ${this.botNumber} pushing high value ${high}`);
                this.pushHigh(high);
            });
...
input.split('\n').forEach((line) => {
    var match;
    if ((match = botRegex.exec(line)) !== null) {
        var botNumber = +match[1];
        var lowIndex = +match[3];
        var highIndex = +match[5];

        var pushLow = match[2] === 'bot' ?
                    (val) => bots[lowIndex].pushValue(val)
                    : (val) => { outputs[lowIndex] = val; console.log(`output ${lowIndex} receives value ${val}`); };
        var pushHigh = match[4] === 'bot' ?
                    (val) => bots[highIndex].pushValue(val)
                    : (val) => { outputs[highIndex] = val; console.log(`output ${highIndex} receives value ${val}`);};
    ...
    } else if ((match = pushValRegex.exec(line)) !== null) {
        var botNumber = +match[2];
        var val = +match[1];

        queuedActions.push(() => {
            bots[botNumber].pushValue(val);
        });
    } else {
        throw new Error('Something went wrong in parsing input');
    }
});
