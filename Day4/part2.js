...
// var input = ['hwbba-gii-eqpvckpogpv-492[pbgiv]'];

function getChecksum(name) {
    var map = {}
    name.split('').filter(a => a !== '-').forEach(function(element) {
        map[element] = 1 + (map[element] | 0);
    });

    var tuples = Object.keys(map).map((key) => { return {letter: key, count: map[key]};});

    tuples.sort((a, b) => {
        return (b.count - a.count) || (a.letter.charCodeAt(0) - b.letter.charCodeAt(0));
    });

    // console.log(name.replace('-', ''));
    // console.log(tuples.map(a => a.letter).slice(0, 5).join(''));
    return tuples.map(a => a.letter).slice(0, 5).join('');
}
...
input.forEach((line) => {
    ...
    if (testRoom(matches[1], matches[3])) {
        var rot = (+matches[2]) % 26;

        var decrypt = matches[1].split('').map(a => {
            if (a === '-') {
                return ' ';
            } else {
                let newChar = a.charCodeAt(0) + rot;
        ...
        console.log(`${decrypt.join('')}[${matches[2]}]`);
    }
});
